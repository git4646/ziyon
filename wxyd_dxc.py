...
新增按照时间来自动选择模式，新增账号详细信息但需在yd值内加上mid值不增也不影响，新增notify青龙推送，修复需重新运行，新增多平台同时运行如需选择多平台运行建议将80行的 'Cookie': 'sid=xxxx' 换成你们的cookie https://request.worktile.com 去当前网页抓取即可
...
def get_jq():
    ...
    headers = {
        'Cookie': 'sid=0e8535564c3d4c79a152e6a2892abdea',
    }
...
def huoqu_xx(c,payload,yd,mid):
   ...
   # print(response)
    ...
    print("""[---------账户名:%s-----------]\n[---------今日阅读次数:%s -----------]\n[---------当前鱼儿:%s -----------]\n[---------累计阅读次数:%s----------–]\n"""%(str(response["uid"]),str(response["dayCount"]),str(response["moneyCurrent"]),str(response["doneWx"])),flush=True)
    if yd!="user":
       del payload["code"]
    return response["uid"]
