...
作者TG:@wcnmsb123
作者QQ:2682921363
本脚本无需手动添加code值，现无需修改定时，如搭配青龙使用效果更加
青龙定时 0 58 11,19 * * *
...
import random
urls=[]
#target_time = datetime.time(11,59,59)#定时时间
...
def lianjje(P00001,code):
    base_url = "https://act.vip.iqiyi.com/supermk/seckill/action/seckill?P00001=%s&code=%s" %(P00001,code)
    return base_url
...
def perform_requests_threadpool(urls):
    with concurrent.futures.ThreadPoolExecutor(max_workers=10000) as executor:  # 控制全部线程最大限度
